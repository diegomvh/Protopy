...
*/
var DI4Y =   1461;  /* days_before_year(5); days in 4 years */
var DI100Y = 36524; /* days_before_year(101); days in 100 years */
var DI400Y = 146097;    /* days_before_year(401); days in 400 years  */

/* ordinal -> year, month, day, considering 01-Jan-0001 as day 1. */
function ord_to_ymd(dt, ordinal) {
    var year, month, day
...
function ymd_to_ord(year, month, day) {
...
/* Ordinal of the Monday starting week 1 of the ISO year.  Week 1 is the
    ...
    */
function iso_week1_monday(year) {
    var first_day = ymd_to_ord(year, 1, 1);    /* ord of 1/1 */
...
/* One step of a mixed-radix conversion.  A "hi" unit is equivalent to
    ...
    * is impossible.
    */
function normalize_pair(dt, hi, lo, factor) {
    assert(factor > 0);
    assert(lo != hi);
...
function normalize_y_m_d(dt) {
    ...
    /* Now only day can be out of bounds (year may also be out of bounds
        ...
        */
    dim = days_in_month(dt['year'], dt['month']);
    if (dt['day'] < 1 || dt['day'] > dim) {
        ...
        if (dt['day'] == 0) {
            ...
            var ordinal = ymd_to_ord(dt['year'], dt['month'], 1) + dt['day'] - 1;
            ord_to_ymd(dt, ordinal);
        }
    }
    assert(dt['month'] > 0);
    assert(dt['day'] > 0);
...
/* Fiddle out-of-bounds months and days so that the result makes some kind
    ...
    */
function normalize_date(dt) {
    var result;

    normalize_y_m_d(dt);
    if (MINYEAR <= dt['year'] && dt['year'] <= MAXYEAR)
        result = 0;
    else {
...
function normalize_datetime(dt) {
    normalize_pair(dt, 'second', 'microsecond', 1000000);
    normalize_pair(dt, 'minute', 'second', 60);
    normalize_pair(dt, 'hour', 'minute', 60);
    normalize_pair(dt, 'day', 'hour', 24);
...
var datetime = type('datetime', [ object ], {
    /***
    ...
    *****/
    ...
    from_date: function(date) {
        ...
        ***/
        return new datetime(
            date.getFullYear(), 
            date.getMonth()+1,  // JavaScript reports January as year 0
            date.getDate(),
            date.getHours(),
            date.getMinutes(),
            date.getSeconds(),
    ...
    from_number: function(number, utc) {
        // Returns a datetime object for now.
        var date = new Date(mumber);
        return datetime.from_date(date, utc);
    },

    //A takes a string, Date object, or datetime object and returns a datetime object
    from: function(obj) {
        if (isinstance(obj, String))
            return datetime.parse(obj);
        if (isinstance(obj, Date))
            return datetime.from_date(obj);
        if (isinstance(obj, Number))
            return datetime.from_number(obj);
    ...
    ***/
    ...
    parse: function(str) {
        ...
        ***/
        ...
        return new datetime(dt['year'], dt['month'], dt['day'], dt['hour'], dt['minute'], dt['second'], dt['microsecond'], dt['tz_hour'], dt['tz_minute']);
    }
}, {
    __init__: function(year, month, day, hour, minute, second, microsecond, tz_hour, tz_minute) {
        /***
    ...
    isoformat: function() {
        return '' + this.year.format('04') + '-' + this.month.format('02') + '-' + this.day.format('02') +
            'T' + this.hour.format('02') + ':' + this.minute.format('02') + ':' + this.second.format('02') + this.microsecond.format('06');
    },
});

var date = type('date', [ object ], {});
var time = type('time', [ object ], {});
/*** datetime.js 0.1
    ...
    ***/
    ...
    addPeriods : function(p1, p2) {
        ...
        ***/
        ...
        for (key in datetime.min) {
            if (!(p2[key]  == undefined) || !(p1[key] == undefined)) {
                result[key] = 0
                if (!(p1[key] == undefined) )
                    result[key] = p1[key]
                if (!(p2[key] == undefined) )
    ...
    subDatetimes : function(dt1, dt2) {
        ...
        ***/
        ...
        if ( !(dt1.tz_hour == undefined) && !(dt2.tz_hour == undefined) )
            datetime.convertTimezone(dt2mod, dt1)
        
        var result = {}
        for (key in datetime.min) {
            if ( !(dt1[key] == undefined) && !(dt2mod[key] == undefined) )
    ...
    convertTimezone : function(dt, new_timezone) {
        ...
        if ( !(dt.tz_hour == undefined) && !(new_timezone.tz_hour == undefined) )
            dt.hour += dt.tz_hour - new_timezone.tz_hour
        if ( !(dt.tz_minute == undefined) && !(new_timezone.tz_minute == undefined) )
    ...
    comparePeriods : function(p1, p2) {
        ...
        */
        for (var i=0; i<datetime.keys.length; i++) {
            ...
            if ( !(p1[key] == undefined) && (p2[key] == undefined) )
                return -1
            else if ( (p1[key] == undefined) && !(p2[key] == undefined) )
                return 1;
        }
        return 0;
    },
    compareDatetimes : function(dt1, dt2) {
        ...
        */
        for (var i=0; i<datetime.keys.length; i++) {
            var key = datetime.keys[i]
            if ( !(dt1[key] == undefined) && !(dt2[key] == undefined) ) {
                var comp = compare(dt1[key], dt2[key])
                if (comp != 0)
                    return comp;
            }
            if ( !(dt1[key] == undefined) && (dt2[key] == undefined) ||
                  (dt1[key] == undefined) && !(dt2[key] == undefined)) {
